      ...
      <HeadlessListbox v-model="currentLocaleCode">
        <div class="relative">
          <HeadlessListboxButton
            class="relative w-full cursor-default rounded-lg py-2 pl-3 pr-8 text-left hover:bg-gray-100 focus:outline-none focus-visible:border-indigo-500 focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 dark:border-gray-700 hover:dark:bg-gray-800 sm:text-sm"
          >
...
const currentLanguage = computed(() => {
  return availableLocales.value.find(
    (locale) => locale.code === currentLocaleCode.value,
  );
});
...
onMounted(() => {
  currentLocaleCode.value =
    typeof useCookie("i18n_redirected").value === "string"
